...
class Visualization(HasTraits):
    ...
    def update_plot(self):
        ...
        beta.display(self.scene.mlab, contours = [1],
                        color = (0, 0, 1), opacity = 0.3)
        msum.display(self.scene.mlab, contours = [1],
                        color = (1, 1, 0), opacity = 0.7)
...
def compute():
    ...
    if (not alpha.isempty()) and (not beta.isempty()):
        ...
        atemp = alpha.get_voxel()
        
        # Getting the total size for convolution
        sza = alpha.get_voxel_shape()
        szb = beta.get_voxel_shape()
        
        temp = zeros(atemp.shape, dtype = 'f')
        x, y, z = atemp.nonzero()
        temp[x.min():x.max()+1, y.min():y.max()+1, z.min():z.max()+1].fill(1)
        beta.set_voxel(temp)
        msum.set_voxel(temp-atemp)
        
        stemp = msum.get_voxel()
        for i in range(sza[0]):
            for j in range(sza[1]):
                for k in range(z.max(), z.min()-1, -1):
                    if stemp[i, j, k] == 0:
                        stemp[i, j, :k].fill(0)
                        break
        
        mdiff.set_voxel(temp - atemp - stemp)
        beta.set_voxel(temp - atemp - stemp)
        msum.set_voxel(temp - atemp - stemp)
